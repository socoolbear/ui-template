/**
 * raphael.draggable plugin 1.0.1 Copyright (c) 2010
 * 
 * @author Matthew Ephraim
 * 
...
(function()
{
  ...
  /**
   * Calling the draggable.enable function on a Raphael canvas will add the
   * functions current and clearCurrent to the canvas. It will also wrap the
   * functions that create elements and sets with extra draggable functionality
   * 
   * @return the canvas
   */
  Raphael.fn.draggable.enable = function()
  {
    ...
    /**
     * Set the current draggable element to the element that was passed in.
     * Otherwise, return the current draggable element (if there is one)
     * 
     * @param {Raphael
     *          element} element (optional) The element to set as the current
     *          draggable
     * 
     * @return the canvas, if an element was passed in, otherwise, the current
     *         draggable
     */
  ...
  /**
   * Wrap each element creation function on the canvas with a custom element
   * function that adds draggable functionality to the newly created element
   * before returning it
   * 
   * @param {Raphael
   *          Canvas} paper The canvas that will have its functions wrapped
   */
   ...
   * 
   * @param {Raphael
   *          canvas} paper The canvas that will have its function wrapped
   * @param {String}
   *          elementType The name of the element function to be wrapped
   */
  ...
  var DraggableExtension = function(element)
  {
    ...
    document.onmouseup = function()
    {
      /**
       * 2011 김봉연
       * chrome browser에서 svg item drag 작업 시 
       * mouse pointer가 text mode로 변하는 문제 해결을 위한
       * document.onselectstart 변경.
       * drag 작업 완료 시 document.onselectstart를 원상 복귀. 
       * @returns true 
       */
  ...
  /**
   * Wrap the set creation function on the canvas with a custom function that
   * adds draggable functionality to the newly created set before returning it
   * 
   * @param {Raphael
   *          canvas} paper The canvas that will have its function wrapped
   */
  ...
  /**
   * Takes a Raphael set and wraps some functions so that they take advantage of
   * the extra draggable functionality
   * 
   * @param {Raphael
   *          set} set The that will have its functions wrapped
   */
